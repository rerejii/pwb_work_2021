...
class LTImageGenerator:
    def __init__(self, model_h, model_w, padding, LT_Generator_model, fin_activate='sigmoid', use_gpu=True,
        ...
        self.interpreter = tf.lite.Interpreter(model_path=LT_Generator_model)
        self.interpreter.allocate_tensors()  # メモリ確保。これはモデル読み込み直後に必須
    ...
    def run(self, img_path, out_path, batch=50, time_out_path=None, ave_time_out_path=None, csv_reset=False):
        ...
        index = 0
        net_time = 0
        H = self.model_h - self.padding
        W = self.model_w - self.padding
        with tqdm(total=math.ceil(len(crop_index)), desc='image generate') as pbar:
            for data in data_iter:
                # convertすると、入力のサイズが失われるので、入力サイズをリサイズする
                next_batch = min(len(crop_index)-index, batch)
                self.interpreter.resize_tensor_input(input_index, [next_batch, 256, 256, 3])
                net_start_time = timefunc.time()
                with tf.device(self.device):
                    # outset = self.generate_img(data)
                    self.interpreter.set_tensor(input_index, data)  # 入力をセットする
                    output = self.interpreter.set_tensor(output_index)  # 出力を指定する (取得する時はoutput())
                    self.interpreter.invoke()  # モデルの処理を実行する
                net_time += timefunc.time() - net_start_time
                for out in output():
                    out = tf.reshape(out, [H, W, 1])
