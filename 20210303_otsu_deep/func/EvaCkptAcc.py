    ...
    def multi_check_step(self, ds_iter, device_list, data_n):
        ...
        for gpu_index in device_list:
            with tf.device('/gpu:%d' % gpu_index):  # gpu単位の処理
                ...
                input_image = nfunc.target_cut_padding(target=input_image, padding=self.net_cls.get_padding())
                ...
                gen_output, gen_loss = self.net_cls.make_binaryimg(input_image, target)
