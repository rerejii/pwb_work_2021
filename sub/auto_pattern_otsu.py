...
args = sys.argv
directorys_1 = 'Z:/hayakawa/work20/dataset2/img'
directorys_2 = glob.glob(directorys_1 + '/17H-0863-1_L*/')
# check_name = 'brank_otsu_l'
check_name = args[1]
# directorys_2 = ['Z:/hayakawa/binary/20210128/unet_onestand/unet_onestand-D']
...
for i in tqdm(range(len(directorys_2)), desc='Processed check pattern'):
    ...
    out_pathset = glob.glob(directorys_2[i] + '/L00??_'+check_name+'.png')
    print(directorys_2[i] + '/L00??_'+check_name+'.png')
    ds_total = np.array([0, 0, 0, 0, 0, 0, 0,])  # ds_total_division, ds_total_short, ds_total_short_area, ds_total_break, ds_total_break_area, ds_total_break_same, ds_total_break_same_area
    ...
    for out_path in out_pathset:  # A, B, C,...
        ...
        os.rename(dirname + '/View.png', dirname+'/View_'+check_name+'.png')

        total_result = check_eveluate(path=directorys_2[i] + '/', csv_name='L'+str(img_n)+'_'+filename+'_PatternResult.csv', file_count=1)
        ds_total += np.array(total_result)  # ds_total_division, ds_total_short, ds_total_short_area, ds_total_break, ds_total_break_area, ds_total_break_same, ds_total_break_same_area
        ...
        #         ['L'+str(img_n), total_result[0], total_result[2], total_result[4] + total_result[6], total_result[4], total_result[6]])
...
with open(directorys_1 + '/'+check_name+'_TotalPatternResult.csv', 'w') as total_file:
    writer_total = csv.writer(total_file, lineterminator='\n')
    writer_total.writerow(['total_division', 'total_short_area',
                            'total_break_area + total_break_same_area', 'total_break_area', 'total_break_same_area'])  
    writer_total.writerow([all_ds_total[0], all_ds_total[2], all_ds_total[4] + all_ds_total[6], all_ds_total[4], all_ds_total[6]])
    writer_total.writerow(['short', 'break+break_same', 'total'])
    writer_total.writerow([all_ds_total[2], all_ds_total[4]+all_ds_total[6], all_ds_total[2] + all_ds_total[4] + all_ds_total[6]])
