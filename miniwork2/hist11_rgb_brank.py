...
for path_i, path in enumerate(img_path):
    ...
    plt.xlim([0, 256])
    plt.title(fname + '_white_only_brank')
    plt.savefig(fname + '_white_only_brank.png')
    plt.figure()
    df = pd.DataFrame(one_histr, columns=color)
    df.to_csv(fname + '_white_only_brank.csv', sep=',')

    for col_i, col in enumerate(color):
        one_histr_inv[:, col_i] = cv2.calcHist([cut_img_inv], [col_i], mask_inv, [256], [0, 256])[:, 0]
        ave_histr_inv[:, col_i] += one_histr[:, col_i]
        plt.plot(one_histr_inv[:, col_i], color=col)
    plt.xlim([0, 256])
    plt.title(fname + '_black_only_brank')
    plt.savefig(fname + '_black_only_brank.png')
    plt.figure()
    df = pd.DataFrame(one_histr_inv, columns=color)
    df.to_csv(fname + '_black_only_brank.csv', sep=',')
...
plt.xlim([0, 256])
plt.title('ave_white_only_brank')
plt.savefig('ave_white_only_brank.png')
plt.figure()
df = pd.DataFrame(ave_histr, columns=color)
df.to_csv('ave_white_only_brank.csv', sep=',')

for col_i, col in enumerate(color):
    plt.plot(ave_histr_inv[:, col_i], color=col)
plt.xlim([0, 256])
plt.title('ave_black_only_brank')
plt.savefig('ave_black_only_brank.png')
plt.figure()
df = pd.DataFrame(ave_histr_inv, columns=color)
df.to_csv('ave_black_only_brank.csv', sep=',')
