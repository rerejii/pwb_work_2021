...
for path_i, path in enumerate(img_path):
    ...
    for i in range(3):
        cut_img[:, :, i] = img[:, :, i] * (mask / 255)
        cut_img_inv[:, :, i] = img[:, :, i] * (mask_inv / 255)

    for col_i, col in enumerate(color):
        one_histr[:, col_i] = cv2.calcHist([cut_img], [col_i], None, [256], [0, 256])[:, 0]
    ...
    df.to_csv(fname + '_white_only.csv', sep=',')

    for col_i, col in enumerate(color):
        one_histr_inv[:, col_i] = cv2.calcHist([cut_img_inv], [col_i], None, [256], [0, 256])[:, 0]
    ...
    df.to_csv(fname + '_black_only.csv', sep=',')
