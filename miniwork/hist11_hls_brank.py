...
for path_i, path in enumerate(img_path):
    ...
    plt.xlim([0, 256])
    plt.title(fname + '_white_only_hls_brank')
    plt.savefig(fname + '_white_only_hls_brank.png')
    plt.figure()
    df = pd.DataFrame(one_histr, columns=hls)
    df.to_csv(fname + '_white_only_hls_brank.csv', sep=',')
    ...
    plt.xlim([0, 256])
    plt.title(fname + '_black_only_hls_brank')
    plt.savefig(fname + '_black_only_hls_brank.png')
    plt.figure()
    df = pd.DataFrame(one_histr_inv, columns=hls)
    df.to_csv(fname + '_black_only_hls_brank.csv', sep=',')
...
plt.xlim([0, 256])
plt.title('ave_white_only_hls_brank')
plt.savefig('ave_white_only_hls_brank.png')
plt.figure()
df = pd.DataFrame(ave_histr, columns=hls)
df.to_csv('ave_white_only_hls_brank.csv', sep=',')

for col_i, col in enumerate(color):
    plt.plot(ave_histr_inv[:, col_i], color=col)
plt.xlim([0, 256])
plt.title('ave_black_only_hls_brank')
plt.savefig('ave_black_only_hls_brank.png')
plt.figure()
df = pd.DataFrame(ave_histr_inv, columns=hls)
df.to_csv('ave_black_only_hls_brank.csv', sep=',')
