...
out_folder_name = OUT_PATH + 'otsu_outbrank_l127_b1'
...
with open('otsu_ret_brank.csv', 'w') as f:
    ...
    for pi in range(len(in_img_path)):
        ...
        ret2_l, img_otsu_l = cv2.threshold(brankimg[:, :, 1], 0, 255, cv2.THRESH_OTSU)
        # out_l = np.zeros([height, width])
        # out_l[img_hls[:, :, 1] >= ret2_l] = 255
        out_l = img_hls[:, :, 1] - ret2_l + 127
        out_l += (img_hls[:, :, 1] >= ret2_l) * ((255 - img_hls[:, :, 1]) * 0.1)
        out_l += (img_hls[:, :, 1] < ret2_l) * ((0 - img_hls[:, :, 1]) * 0.1)

        # ret2_s, img_otsu_s = cv2.threshold(brankimg[:, :, 2], 0, 255, cv2.THRESH_OTSU)
        # out_s = np.zeros([height, width])
        # out_s[img_hls[:, :, 2] >= ret2_s] = 255


        # img_otsu_l = cv2.resize(out_l, (int(width * 0.125), int(height * 0.125)))
        # img_otsu_s = cv2.resize(out_s, (int(width * 0.125), int(height * 0.125)))

        cv2.imwrite(os.path.splitext(path)[0] + '_brank_otsu_l127_b1.png', out_l)
        img2 = cv2.resize(out_l, (int(width * 0.125), int(height * 0.125)))
        cv2.imwrite(out_folder_name + '/' + os.path.splitext(os.path.basename(path))[0] + '_brank_otsu_l127_b1' + '_d125.png', img2)
