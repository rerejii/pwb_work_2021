...
out_folder_name = 'otsu_outbrank_h'
...
for pi in range(len(in_img_path)):
    ...
    ret2, _ = cv2.threshold(brankimg[:, :, 2], 0, 255, cv2.THRESH_OTSU)
    out = np.zeros([height, width])
    out[img_hls[:, :, 0] >= ret2] = 255
    cv2.imwrite(os.path.splitext(path)[0] + '_brank_otsu_h.png', out)


    img_otsu = cv2.resize(out, (int(width * 0.125), int(height * 0.125)))
    cv2.imwrite(out_folder_name + '/' + os.path.splitext(os.path.basename(path))[0] + '_brank_otsu_h_d125.png', img_otsu)
