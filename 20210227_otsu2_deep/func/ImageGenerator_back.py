...
class ImageGenerator:
    def __init__(self, model_h, model_w, padding, Generator_model, fin_activate='sigmoid', use_gpu=True,
        ...
        if use_gpu:
            with tf.device(self.device):
                zero_data = np.zeros(shape=[1, self.model_h, self.model_w, 4], dtype=np.float32)  # 空データ
    ...
    def _img_size_norm(self, img, ch=3):
        ...
        flame = np.zeros([flame_h, flame_w, ch], dtype=np.uint8)
    ...
    def generate_img(self, data):
        # data_list = [data]
        # for i in range(self.deep_loop):
        #     gen_out = self.Generator(data_list[-1], training=False)
        #     gen_bin = self.binary_from_data(gen_out, self.fin_activate)
        #     next_data = tf.concat([data_list[-1][:, :, :, 0:3], tf.cast(gen_bin, tf.float32)], -1)
    ...
    def run(self, img_path, out_path, hls_path, batch=50, time_out_path=None, ave_time_out_path=None, csv_reset=False):
        ...
        img_byte = tf.io.read_file(hls_path)
        hls_img = tf.image.decode_png(img_byte, channels=1)
        ...
        in_img = tf.concat([in_img, hls_img], -1)
        norm_img, size = self._img_size_norm(in_img, ch=4)
        # norm_img, size = self._img_size_norm(in_img, ch=3)  # 画像サイズを調整する
        # hls_img, size = self._img_size_norm(hls_img, ch=1)  # 画像サイズを調整する
        # norm_img = tf.concat([norm_img, hls_img], -1)
        # print(norm_img)
        # sys.exit

        # cv2.imwrite('E:/hls.png', hls_img)
        # print(hls_img[10000:10100, 10000:10100])
        # sys.exit()

        # 切り出す画像の座標を求める
        crop_index = self.check_crop_index(norm_img)

        # hsv_img = tf.image.rgb_to_hsv(norm_img)
        # norm_img = tf.concat([norm_img, hsv_img[:, :, 1:]], -1)

        # _hsv_image = tf.image.rgb_to_hsv(_sample_image)
        # _sample_image = tf.concat([_sample_image, _hsv_image[:, :, 1:]], -1)
        ...
        # import cv2
        # hls_img = cv2.cvtColor(norm_img, cv2.COLOR_RGB2HLS)
        # # _sample_image = tf.concat([_sample_image, _hls_image[:, :, 1:]], -1)
        # norm_img = tf.concat([norm_img, hls_img[:, :, 1:]], -1)

        # 画像切り出し用関数
        def _cut_img(img_index):
            # cut = tf.slice(norm_img, [img_index[0], img_index[1], 0],
            #                [self.model_h, self.model_w, 4])
            cut = norm_img[img_index[0]: img_index[0]+self.model_h, img_index[1]: img_index[1]+self.model_w, :]
