...
# ************ クラス外呼び出し関数 ************
    ...
    def generator_loss(self, gen_output, target, weight):
        ...
        gen_loss = self.loss_object(y_true=target, y_pred=gen_output) * weight
        return gen_loss
    ...
    def multi_train_step(self, ds, device_list):
        ...
        for gpu_index in device_list:
            with tf.device('/gpu:%d' % gpu_index): # gpu単位の処理
                ...
                with tf.GradientTape() as gen_tape:
                    ...
                    data_list = [input_image]
                    gen_loss = None
                    for i in range(self.deep_loop):
                        # data_list.append(generator(data_list[-1], training=True))
                        gen_out = generator(data_list[-1], training=True)
                        if gen_loss is None:
                            gen_loss = self.generator_loss(gen_out, target, weight=1)
                        else:
                            gen_loss += self.generator_loss(gen_out, target, weight=1)
                        gen_bin = self.binary_from_data(gen_out, label='output')
                        gen_bin = tf.cast(gen_bin, tf.float32)
                        next_data = tf.concat([data_list[-1][:, :, :, 0:3], gen_bin], -1)
        ...
        return gen_out, err_count, c_weight, gen_loss
    ...
    # ========== check_accuracy関数 ================================================================================
    # ==============================================================================================================
    def make_binaryimg(self, input_image, gen_target=None):
        data_list = [input_image]
        generator = self.get_generator()
        gen_loss = 0.
        for i in range(self.deep_loop):
            gen_out = generator(data_list[-1], training=False)
            gen_bin = self.binary_from_data(gen_out, label='output')
            gen_bin = tf.cast(gen_bin, tf.float32)
            next_data = tf.concat([data_list[-1][:, :, :, 0:3], gen_bin], -1)
            data_list.append(next_data)
        if gen_target is not None:
            gen_loss += self.generator_loss(gen_bin, gen_target, weight=1)
        # out = [data_list[-1]]
        out = [gen_out]
        if gen_target is not None:
            out.append(gen_loss)
