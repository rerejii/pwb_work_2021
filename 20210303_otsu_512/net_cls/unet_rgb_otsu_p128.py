...
# ************ クラス外呼び出し関数 ************
    ...
    def multi_train_step(self, ds, device_list):
        ...
        for gpu_index in device_list:
            with tf.device('/gpu:%d' % gpu_index): # gpu単位の処理
                ...
                with tf.GradientTape() as gen_tape:
                    ...
                    weight = nfunc.target_cut_padding(target=weight, padding=self.get_padding())
                    distance = nfunc.target_cut_padding(target=distance, padding=self.get_padding())
                    # Generatorによる画像生成
                    generator = self.get_generator()
                    gen_output = generator(input_image, training=True)
                    gen_output = nfunc.target_cut_padding(target=gen_output, padding=self.get_padding())
                    # Discriminatorによる判定
        ...
        return gen_output, err_count, c_weight, gen_loss
    ...
    def train_step(self, ds, device_list, rate=1):
        ...
        return gen_output, err_count, c_weight, (gen_loss * use_rate)
