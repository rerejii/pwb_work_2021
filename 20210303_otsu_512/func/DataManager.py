...
class DataManager():
    ...
    def get_inited_iter(self):
        ...
        # _dataset = _dataset.map(self._cut_49)
    ...
    def _random_crop(self, _sample_image, _answer_image, _weight_image, _dis_image, parse):
        stacked_image = tf.stack([_sample_image[:, :, 0], _sample_image[:, :, 1], _sample_image[:, :, 2], _sample_image[:, :, 3],
                                  _answer_image[:, :, 0], _weight_image[:, :, 0], _dis_image[:, :, 0]], axis=0)
        cropped_image = tf.image.random_crop(stacked_image, size=[7, 512, 512])
        _sample_image = tf.stack([cropped_image[0], cropped_image[1], cropped_image[2], cropped_image[3]], axis=2)
        _answer_image = tf.expand_dims(cropped_image[4], axis=-1)
        _weight_image = tf.expand_dims(cropped_image[5], axis=-1)
        _dis_image = tf.expand_dims(cropped_image[6], axis=-1)
    ...
    # def _cut_49(self, _sample_image, _answer_image, _weight_image, _dis_image, parse):
    #     _sample_image = _sample_image[49:-49, 49:-49, :]
    #     _answer_image = _answer_image[49:-49, 49:-49]
    #     _weight_image = _weight_image[49:-49, 49:-49]
    #     _dis_image = _dis_image[49:-49, 49:-49]
    #     return _sample_image, _answer_image, _weight_image, _dis_image, parse
    ...
    def _parse_function(self, _data):
        _features = {'sample_path': tf.io.FixedLenFeature((), tf.string, default_value=''),
                     ...
                     'brank_otsu_l_path': tf.io.FixedLenFeature((), tf.string, default_value=''),
    ...
    def _image_from_path(self, _parsed_features):
        ...
        _otsu_png_bytes = tf.io.read_file(self.img_root + '/' + _parsed_features['brank_otsu_l_path'])
        _otsu_image = tf.image.decode_png(_otsu_png_bytes, channels=1, dtype=tf.dtypes.uint8, )
        _sample_image = tf.concat([_sample_image, _otsu_image], -1)
