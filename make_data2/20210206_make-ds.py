...
OUT_DS_FOLDER = 'DR-20210208-valid'+str(PADDING)+'-more'
...
def pre_proc(path):
    ...
    sam_norm, ans_norm, weight_norm, dis_norm, bound_norm, hls_norm, otsu127_l_norm, otsu127_s_norm, brank_otsu_l_norm,\
        brank_otsu_l_c3_norm, brank_otsu_l_c5_norm =\
        img_size_norm(sam_img, ans_img, weight_img, dis_img, bound_img, hls_img, otsu127_l_img, otsu127_s_img, brank_otsu_l_img,
                      brank_otsu_l_c3_img, brank_otsu_l_c5_img)
    # 画像の切り出し座標を求める
    crop_index = check_crop_index(sam_img)
    return sam_norm, ans_norm, weight_norm, dis_norm, bound_norm, hls_norm, otsu127_l_norm, otsu127_s_norm, brank_otsu_l_norm, brank_otsu_l_c3_norm, brank_otsu_l_c5_norm, crop_index


def make_recode(h, w, img_name, group_name, sample_path, answer_path, weight_path, dis_path, bound_path, hls_path, otsu127_l_path, otsu127_s_path, brank_otsu_l_path, brank_otsu_l_c3_path, brank_otsu_l_c5_path, stand):
    record = tf.train.Example(features=tf.train.Features(feature={
        ...
        'weight_path': tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[weight_path.encode()])),
        'dis_path': tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[dis_path.encode()])),
        'bound_path': tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[bound_path.encode()])),
        'hls_path': tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[hls_path.encode()])),
        'otsu127_l_path': tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[otsu127_l_path.encode()])),
        'otsu127_s_path': tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[otsu127_s_path.encode()])),
        'brank_otsu_l_path': tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[brank_otsu_l_path.encode()])),
        'brank_otsu_l_c3_path': tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[brank_otsu_l_c3_path.encode()])),
        'brank_otsu_l_c5_path': tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[brank_otsu_l_c5_path.encode()])),
...
def img_size_norm(sam_img, ans_img, weight_img, dis_img, bound_img, hls_img, otsu127_l_img, otsu127_s_img,
                  brank_otsu_l_img, brank_otsu_l_c3_img, brank_otsu_l_c5_img):
    ...
    weight_flame = np.zeros([flame_h, flame_w, 1], dtype=np.uint8)
    dis_flame = np.ones([flame_h, flame_w, 1], dtype=np.uint8) * 255  # 画素値が距離を示すため、追加する画素は255(MAX)
    bound_flame = np.zeros([flame_h, flame_w, 1], dtype=np.uint8)
    otsu127_l_flame = np.zeros([flame_h, flame_w, 1], dtype=np.uint8)
    otsu127_s_flame = np.zeros([flame_h, flame_w, 1], dtype=np.uint8)
    hls_flame = np.zeros([flame_h, flame_w, 3], dtype=np.uint8)
    brank_otsu_l_flame = np.zeros([flame_h, flame_w, 1], dtype=np.uint8)
    brank_otsu_l_c3_flame = np.zeros([flame_h, flame_w, 1], dtype=np.uint8)
    brank_otsu_l_c5_flame = np.zeros([flame_h, flame_w, 1], dtype=np.uint8)
    ...
    weight_flame[top:bottom, left:right] = weight_img
    dis_flame[top:bottom, left:right] = dis_img
    bound_flame[top:bottom, left:right] = bound_img
    otsu127_l_flame[top:bottom, left:right] = otsu127_l_img
    otsu127_s_flame[top:bottom, left:right] = otsu127_s_img
    hls_flame[top:bottom, left:right, :] = hls_img
    brank_otsu_l_flame[top:bottom, left:right, :] = brank_otsu_l_img
    brank_otsu_l_c3_flame[top:bottom, left:right, :] = brank_otsu_l_c3_img
    brank_otsu_l_c5_flame[top:bottom, left:right, :] = brank_otsu_l_c5_img
    return sam_flame, ans_flame, weight_flame, dis_flame, bound_flame, hls_flame, otsu127_l_flame, otsu127_s_flame, brank_otsu_l_flame, brank_otsu_l_c3_flame, brank_otsu_l_c5_flame
...
for pi in range(len(in_img_path)):
    ...
    sam_norm, ans_norm, weight_norm, dis_norm, bound_norm, hls_norm, otsu127_l_norm, otsu127_s_norm, brank_otsu_l_norm, brank_otsu_l_c3_norm, brank_otsu_l_c5_norm, crop_index = pre_proc(in_img_path[pi])
    # 画像プロパティ
    ...
    dis_folder = img_root_folder + group_name + '/Distance/'
    bound_folder = img_root_folder + group_name + '/Boundary/'
    hls_folder = img_root_folder + group_name + '/HLS/'
    otsu127_l_folder = img_root_folder + group_name + '/Otsu127_l/'
    otsu127_s_folder = img_root_folder + group_name + '/Otsu127_s/'
    brank_otsu_l_folder = img_root_folder + group_name + '/Brank_otsu_l/'
    brank_otsu_l_c3_folder = img_root_folder + group_name + '/Brank_otsu_l_c3/'
    brank_otsu_l_c5_folder = img_root_folder + group_name + '/Brank_otsu_l_c5/'
    ...
    os.makedirs(otsu127_l_folder, exist_ok=True)
    os.makedirs(otsu127_s_folder, exist_ok=True)
    os.makedirs(brank_otsu_l_folder, exist_ok=True)
    os.makedirs(brank_otsu_l_c3_folder, exist_ok=True)
    os.makedirs(brank_otsu_l_c5_folder, exist_ok=True)
    ...
    with tqdm(total=len(crop_index), desc='Processed') as pbar:
        ...
        for ci in range(len(crop_index)):
            ...
            weight_crop = weight_norm[crop_top: crop_top + CROP_H + (PADDING * 2),
                       crop_left: crop_left + CROP_H + (PADDING * 2)]
            dis_crop = dis_norm[crop_top: crop_top + CROP_H + (PADDING * 2),
                       crop_left: crop_left + CROP_H + (PADDING * 2)]
            bound_crop = bound_norm[crop_top: crop_top + CROP_H + (PADDING * 2),
                       crop_left: crop_left + CROP_H + (PADDING * 2)]
            hls_crop = hls_norm[crop_top: crop_top + CROP_H + (PADDING * 2),
                       crop_left: crop_left + CROP_W + (PADDING * 2), :]
            otsu127_l_crop = otsu127_l_norm[crop_top: crop_top + CROP_H + (PADDING * 2),
                       crop_left: crop_left + CROP_H + (PADDING * 2)]
            otsu127_s_crop = otsu127_s_norm[crop_top: crop_top + CROP_H + (PADDING * 2),
                       crop_left: crop_left + CROP_H + (PADDING * 2)]
            brank_otsu_l_crop = brank_otsu_l_norm[crop_top: crop_top + CROP_H + (PADDING * 2),
                       crop_left: crop_left + CROP_H + (PADDING * 2)]
            brank_otsu_l_c3_crop = brank_otsu_l_c3_norm[crop_top: crop_top + CROP_H + (PADDING * 2),
                       crop_left: crop_left + CROP_H + (PADDING * 2)]
            brank_otsu_l_c5_crop = brank_otsu_l_c5_norm[crop_top: crop_top + CROP_H + (PADDING * 2),
                       crop_left: crop_left + CROP_H + (PADDING * 2)]
            ...
            dis_out_path = dis_folder + '/Distance_X' + str_X + '_Y' + str_Y + '.png'
            bound_out_path = bound_folder + '/Boundary_X' + str_X + '_Y' + str_Y + '.png'
            hls_out_path = hls_folder + '/HLS_X' + str_X + '_Y' + str_Y + '.png'
            otsu127_l_out_path = otsu127_l_folder + '/Otsu127_l_X' + str_X + '_Y' + str_Y + '.png'
            otsu127_s_out_path = otsu127_s_folder + '/Otsu127_s_X' + str_X + '_Y' + str_Y + '.png'
            brank_otsu_l_out_path = brank_otsu_l_folder + '/Brank_otsu_l_X' + str_X + '_Y' + str_Y + '.png'
            brank_otsu_l_c3_out_path = brank_otsu_l_c3_folder + '/Brank_otsu_l_c3_X' + str_X + '_Y' + str_Y + '.png'
            brank_otsu_l_c5_out_path = brank_otsu_l_c5_folder + '/Brank_otsu_l_c5_X' + str_X + '_Y' + str_Y + '.png'
            ...
            dis_record_path = group_name + '/Distance' + '/Distance_X' + str_X + '_Y' + str_Y + '.png'
            bound_record_path = group_name + '/Boundary' + '/Boundary_X' + str_X + '_Y' + str_Y + '.png'
            hls_record_path = group_name + '/HLS' + '/HLS_X' + str_X + '_Y' + str_Y + '.png'
            otsu127_l_record_path = group_name + '/Otsu127_l' + '/Otsu127_l_X' + str_X + '_Y' + str_Y + '.png'
            otsu127_s_record_path = group_name + '/Otsu127_s' + '/Otsu127_s_X' + str_X + '_Y' + str_Y + '.png'
            brank_otsu_l_record_path = group_name + '/Brank_otsu_l' + '/Brank_otsu_l_X' + str_X + '_Y' + str_Y + '.png'
            brank_otsu_l_c3_record_path = group_name + '/Brank_otsu_l_c3' + '/Brank_otsu_l_c3_X' + str_X + '_Y' + str_Y + '.png'
            brank_otsu_l_c5_record_path = group_name + '/Brank_otsu_l_c5' + '/Brank_otsu_l_c5_X' + str_X + '_Y' + str_Y + '.png'
            ...
            if not os.path.exists(weight_out_path):
                weight_byte = tf.image.encode_png(weight_crop)
                tf.io.write_file(filename=weight_out_path, contents=weight_byte)
            if not os.path.exists(dis_out_path):
                dis_byte = tf.image.encode_png(dis_crop)
                tf.io.write_file(filename=dis_out_path, contents=dis_byte)
            if not os.path.exists(bound_out_path):
                bound_byte = tf.image.encode_png(bound_crop)
                tf.io.write_file(filename=bound_out_path, contents=bound_byte)
            if not os.path.exists(hls_out_path):
                hls_byte = tf.image.encode_png(hls_crop)
                tf.io.write_file(filename=hls_out_path, contents=hls_byte)
            if not os.path.exists(brank_otsu_l_out_path):
                hls_byte = tf.image.encode_png(brank_otsu_l_crop)
                tf.io.write_file(filename=brank_otsu_l_out_path, contents=hls_byte)

            old_brank_otsu_l_c3_out_path = brank_otsu_l_folder + '/Brank_otsu_l_c3_X' + str_X + '_Y' + str_Y + '.png'
            if os.path.exists(old_brank_otsu_l_c3_out_path):
                shutil.move(old_brank_otsu_l_c3_out_path, brank_otsu_l_c3_out_path)
            elif not os.path.exists(brank_otsu_l_c3_out_path):
                hls_byte = tf.image.encode_png(brank_otsu_l_c3_crop)
                tf.io.write_file(filename=brank_otsu_l_c3_out_path, contents=hls_byte)

            old_brank_otsu_l_c5_out_path = brank_otsu_l_folder + '/Brank_otsu_l_c5_X' + str_X + '_Y' + str_Y + '.png'
            if os.path.exists(old_brank_otsu_l_c5_out_path):
                shutil.move(old_brank_otsu_l_c5_out_path, brank_otsu_l_c5_out_path)
            elif not os.path.exists(brank_otsu_l_c5_out_path):
                hls_byte = tf.image.encode_png(brank_otsu_l_c5_crop)
                tf.io.write_file(filename=brank_otsu_l_c5_out_path, contents=hls_byte)

            # print(brank_otsu_l_c3_out_path)
            # if not os.path.exists(brank_otsu_l_c3_out_path):
            #     hls_byte = tf.image.encode_png(brank_otsu_l_c3_crop)
            #     tf.io.write_file(filename=brank_otsu_l_c3_out_path, contents=hls_byte)
            # if not os.path.exists(brank_otsu_l_c5_out_path):
            #     hls_byte = tf.image.encode_png(brank_otsu_l_c5_crop)
            #     tf.io.write_file(filename=brank_otsu_l_c5_out_path, contents=hls_byte)

            otsu127_l_byte = tf.image.encode_png(otsu127_l_crop)
            tf.io.write_file(filename=otsu127_l_out_path, contents=otsu127_l_byte)

            old_otsu127_l_out_path = bound_folder + '/Otsu127_l_X' + str_X + '_Y' + str_Y + '.png'
            old_otsu127_s_out_path = bound_folder + '/Otsu127_s_X' + str_X + '_Y' + str_Y + '.png'

            # if os.path.exists(old_otsu127_l_out_path):
            #     shutil.move(old_otsu127_l_out_path, otsu127_l_out_path)
            # elif not os.path.exists(otsu127_l_out_path):
            #     otsu127_l_byte = tf.image.encode_png(otsu127_l_crop)
            #     tf.io.write_file(filename=otsu127_l_out_path, contents=otsu127_l_byte)
            if os.path.exists(old_otsu127_s_out_path):
                shutil.move(old_otsu127_s_out_path, otsu127_s_out_path)
            elif not os.path.exists(otsu127_s_out_path):
                otsu127_s_byte = tf.image.encode_png(otsu127_s_crop)
                tf.io.write_file(filename=otsu127_s_out_path, contents=otsu127_s_byte)
            recode = make_recode(h, w, img_name, group_name, sam_record_path, ans_record_path, weight_record_path,
                                 dis_record_path, bound_record_path, hls_record_path, otsu127_l_record_path,
                                 otsu127_s_record_path, brank_otsu_l_record_path,
                                 brank_otsu_l_c3_record_path, brank_otsu_l_c5_record_path, df_stand.loc[img_name].values)
