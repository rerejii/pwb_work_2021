...
OUT_DS_FOLDER = 'DR-256-dis-hls-otus-onestand-weight-valid'+str(PADDING)+'-more'
...
def pre_proc(path):
    ...
    sam_norm, ans_norm, weight_norm, dis_norm, bound_norm, hls_norm, otsu127_l_norm, otsu127_s_norm = img_size_norm(sam_img, ans_img, weight_img, dis_img, bound_img, hls_img, otsu127_l_img, otsu127_s_img)
    # 画像の切り出し座標を求める
    crop_index = check_crop_index(sam_img)
    return sam_norm, ans_norm, weight_norm, dis_norm, bound_norm, hls_norm, otsu127_l_norm, otsu127_s_norm, crop_index


def make_recode(h, w, img_name, group_name, sample_path, answer_path, weight_path, dis_path, bound_path, hls_path, otsu127_l_path, otsu127_s_path, stand):
...
def img_size_norm(sam_img, ans_img, weight_img, dis_img, bound_img, hls_img, otsu127_l_img, otsu127_s_img):
    ...
    return sam_flame, ans_flame, weight_flame, dis_flame, bound_flame, hls_flame, otsu127_l_flame, otsu127_s_flame
...
for pi in range(len(in_img_path)):
    ...
    sam_norm, ans_norm, weight_norm, dis_norm, bound_norm, hls_norm, otsu127_l_norm, otsu127_s_norm, crop_index = pre_proc(in_img_path[pi])
    # 画像プロパティ
    ...
    with tqdm(total=len(crop_index), desc='Processed') as pbar:
        ...
        for ci in range(len(crop_index)):
            ...
            if not os.path.exists(hls_record_path):
            ...
            recode = make_recode(h, w, img_name, group_name, sam_record_path, ans_record_path, weight_record_path, dis_record_path, bound_record_path, hls_record_path, otsu127_l_record_path, otsu127_s_record_path, df_stand.loc[img_name].values)
