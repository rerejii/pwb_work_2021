...
OUT_IMG_FOLDER = 'Img-256-gray-valid'+str(PADDING)+'-more'
OUT_DS_FOLDER = 'DR-256-gray-valid'+str(PADDING)+'-more'
...
def pre_proc(path):
    ...
    # グレイスケール化
    sam_img = cv2.cvtColor(sam_img, cv2.COLOR_RGB2GRAY)
...
def make_recode(h, w, img_name, group_name, sample_path, answer_path, average_path):
    record = tf.train.Example(features=tf.train.Features(feature={
        ...
        'average_path': tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[average_path.encode()])),
    }))
    return record
...
def img_size_norm(sam_img, ans_img):
    ...
    origin_h, origin_w = sam_img.shape
    ...
    sam_flame = np.zeros([flame_h, flame_w, 1], dtype=np.uint8)
    ...
    sam_flame[top:bottom, left:right, :] = sam_img.reshape([origin_h, origin_w, 1])
    ans_flame[top:bottom, left:right, :] = ans_img.reshape([origin_h, origin_w, 1])
    return sam_flame, ans_flame


def check_crop_index(norm_img):
    norm_h, norm_w = norm_img.shape
...
def ave_gray_img(img, mask,):
    o_mask = (mask == True)
    o_pass_img = np.array(img * o_mask, dtype=np.uint8)
    o_count_n = np.sum(o_mask)
    o_color = round(np.sum(o_pass_img, dtype=np.uint64) / o_count_n) if o_count_n != 0 else 0
    z_mask = (mask == False)
    z_pass_img = np.array(img * z_mask, dtype=np.uint8)
    z_count_n = np.sum(z_mask)
    z_color = round(np.sum(z_pass_img, dtype=np.uint64) / z_count_n) if z_count_n != 0 else 0
    return np.array((o_mask * o_color) + (z_mask * z_color), dtype=np.uint8)
...
for pi in range(len(in_img_path)):
    ...
    # 画像関連の前処理
    sam_norm, ans_norm, crop_index = pre_proc(in_img_path[pi])
    # 画像プロパティ
    ...
    ave_folder = img_root_folder + group_name + '/Average/'
    ...
    with tqdm(total=len(crop_index), desc='Processed') as pbar:
        ...
        for ci in range(len(crop_index)):
            ...
            ave_crop = ave_gray_img(img=sam_crop, mask=(ans_crop > 127))
            # ave_crop = ans_norm[crop_top: crop_top + CROP_H + (PADDING * 2),
            #            crop_left: crop_left + CROP_H + (PADDING * 2)]
            ...
            ave_out_path = ave_folder + '/Average_X' + str_X + '_Y' + str_Y + '.png'
            ...
            ave_record_path = group_name + '/Average' + '/Average_X' + str_X + '_Y' + str_Y + '.png'
            ...
            if not os.path.exists(ave_out_path):
                ave_byte = tf.image.encode_png(ave_crop)
                tf.io.write_file(filename=ave_out_path, contents=ave_byte)
            recode = make_recode(h, w, img_name, group_name, sam_record_path, ans_record_path, ave_record_path)
