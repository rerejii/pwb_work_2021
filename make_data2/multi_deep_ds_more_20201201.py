...
OUT_DS_FOLDER = 'DR-256-Otsu-valid'+str(PADDING)+'-more'
...
def pre_proc(path):
    ...
    deep_img = read_image(path=os.path.splitext(path)[0] + '_deep_19.png', ch=3)
    # 画像をうまく切り取れるようにリサイズする
    sam_norm, ans_norm, deep_norm = img_size_norm(sam_img, ans_img, deep_img)
    # 画像の切り出し座標を求める
    crop_index = check_crop_index(sam_img)
    return sam_norm, ans_norm, deep_norm, crop_index


def make_recode(h, w, img_name, group_name, sample_path, answer_path, deep_path):
    record = tf.train.Example(features=tf.train.Features(feature={
        ...
        'deepotsu_path': tf.train.Feature(
            bytes_list=tf.train.BytesList(value=[deep_path.encode()])),
    }))
    return record
...
for pi in range(len(in_img_path)):
    ...
    # 画像関連の前処理
    sam_norm, ans_norm, deep_norm, crop_index = pre_proc(in_img_path[pi])
    # 画像プロパティ
    ...
    deep_folder = img_root_folder + group_name + '/DeepOtsu/'
    ...
    with tqdm(total=len(crop_index), desc='Processed') as pbar:
        ...
        for ci in range(len(crop_index)):
            ...
            deep_out_path = deep_folder + '/DeepOtsu_X' + str_X + '_Y' + str_Y + '.png'
            ...
            deep_record_path = group_name + '/DeepOtsu' + '/DeepOtsu_X' + str_X + '_Y' + str_Y + '.png'
            ...
            if not os.path.exists(deep_out_path):
                deep_byte = tf.image.encode_png(deep_crop)
                tf.io.write_file(filename=deep_out_path, contents=deep_byte)
            recode = make_recode(h, w, img_name, group_name, sam_record_path, ans_record_path, deep_record_path)
