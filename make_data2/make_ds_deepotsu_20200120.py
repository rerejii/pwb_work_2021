...
def pre_proc(path):
    ...
    # グレイスケール化
    sam_img = cv2.cvtColor(sam_img, cv2.COLOR_BGR2GRAY)
...
def ave_gray_img(mask, img,):
    o_mask = (mask == True)
    o_pass_img = np.array(img * o_mask, dtype=np.uint8)
    o_count_n = np.sum(mask)
    o_color = round(np.sum(o_pass_img) / o_count_n)
    z_mask = (mask == False)
    z_pass_img = np.array(img * z_mask, dtype=np.uint8)
    z_count_n = np.sum(mask)
    z_color = round(np.sum(z_pass_img) / z_count_n)
    return (o_mask * o_color) + (z_mask * z_color)
