...
PADDING = 0
RANDOM_SEED = 1
SAMPLE_FILE = 'E:/work/myTensor/dataset/17H-0863-1_L0002-new/L0002.png'
ANSWER_FILE = 'E:/work/myTensor/dataset/17H-0863-1_L0002-new/L0002_bin.png'
...
for set in range(4):
    # 使用した画像を確認するもの
    # c_learn_flame = np.zeros([flame_h, flame_w, 3], dtype=np.uint8)
    # c_test_flame = np.zeros([flame_h, flame_w, 3], dtype=np.uint8)
    # c_total_flame = np.zeros([flame_h, flame_w, 3], dtype=np.uint8)
    # set毎のフォルダを生成
    ...
    # 切り出せる画像数を求める
    h_count = int((flame_h - (PADDING * 2)) / (CROP_H))
    w_count = int((flame_w - (PADDING * 2)) / (CROP_W))
    with tqdm(total=h_count, desc=SETNAME[set] + ' set Processed') as pbar:
        for h in range(h_count):
            ...
            for w in range(w_count):
                ...
                # if check_test:
                #     os.makedirs(SETNAME[set]+'/'+'test', exist_ok=True)
                #     cv2.imwrite(SETNAME[set]+'/'+'test' + '/Sample_X' + str_X + '_Y' + str_Y + '.png', sam_crop)

                if not check_test:
                    learn_ds_w.write(record.SerializeToString())
                #     c_learn_flame[crop_top: crop_bottom, crop_left: crop_right, :] = sam_crop
                else:
                    test_ds_w.write(record.SerializeToString())
                #     c_test_flame[crop_top: crop_bottom, crop_left: crop_right, :] = sam_crop
                # c_total_flame[crop_top: crop_bottom, crop_left: crop_right, :] = sam_crop
                # カウント
                test_count += check_test
                learn_count += not check_test
            pbar.update(1)  # プロセスバーを進行
        # 1セット終了時処理
        # os.makedirs('check_img/'+SETNAME[set], exist_ok=True)
        # cv2.imwrite('check_img/'+SETNAME[set]+'/learn.png', c_learn_flame)
        # cv2.imwrite('check_img/'+SETNAME[set]+'/test.png', c_test_flame)
        # cv2.imwrite('check_img/'+SETNAME[set]+'/total.png', c_total_flame)
