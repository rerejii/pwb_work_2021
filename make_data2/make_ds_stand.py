...
STD_CSV = 'Z:/hayakawa/work/myTensor/20201026/img-std.csv'
...
def make_recode(h, w, img_name, group_name, sample_path, answer_path, stand_list):
    record = tf.train.Example(features=tf.train.Features(feature={
        ...
        'std_val': tf.train.Feature(float_list=tf.train.FloatList(value=stand_list)),
    }))
    return record
...
# 画像検索
in_img_path = [path for path in glob.glob(origin_folder) if path.find('_bin') is -1 and path.find('_cutout') is -1]
print(in_img_path)
...
for pi in range(len(in_img_path)):
    # sys.exit()
    fi = in_img_path[pi].find('L00')
    print(in_img_path[pi][fi+1:fi+5])
    img_index = int(in_img_path[pi][fi+1:fi+5]) - 1
    df = pd.read_csv(STD_CSV)
    # print(df['R_std'].iloc[img_index])
    # sys.exit()
    ...
    with tqdm(total=len(crop_index), desc='Processed') as pbar:
        ...
        for ci in range(len(crop_index)):
            ...
            stand_list = np.array(stand_list, dtype='float32')

            recode = make_recode(h, w, img_name, group_name, sam_record_path, ans_record_path, stand_list)
