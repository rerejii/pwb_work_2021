...
class ImageGenerator:
    def __init__(self, model_h, model_w, padding, Generator_model, fin_activate='sigmoid', use_gpu=True,
        ...
        if use_gpu:
            with tf.device(self.device):
                zero_data = np.zeros(shape=[1, self.model_h, self.model_w, 5], dtype=np.float32)  # 空データ
    ...
    def run(self, img_path, out_path, hls_path, batch=50, time_out_path=None, ave_time_out_path=None, csv_reset=False):
        ...
        hls_img = tf.image.decode_png(img_byte, channels=3)
        ...
        hls_img, size = self._img_size_norm(hls_img)  # 画像サイズを調整する
