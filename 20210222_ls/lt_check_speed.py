...
# ========== GPU設定 ==========
args = sys.argv
DEVICE_NUMBER_STR = args[6]  # 使用するGPU設定
...
#  '20200223'
OUT_ROOT_FOLDER_NAME = '20201027'
#  '20200223'
EPOCH_OUT_ROOT_FOLDER_NAME = '20201027'
#  DR*
...
from LTImageGenerator import LTImageGenerator
...
# for DATASET in ['C']:
    ...
    # model = gen
    in_l = model._layers[0]
    print(in_l.input_shape)
    # model._layers[0].input_shape = [None,None,None,3]
    # in_l = model._layers[2]
    # in_l.input_shape = [None, IMAGE_HEIGHT, IMAGE_WIDTH, 3]
    print(in_l.input_shape)
    # model.summary()
    # in_l.input_shape = [None, IMAGE_HEIGHT, IMAGE_WIDTH, 3]
    # Create a new input layer to replace the (None,None,None,3) input layer :
    print(model._layers[0])
    input_layer = tf.keras.layers.Input(shape=[IMAGE_HEIGHT, IMAGE_WIDTH, 3], name='input_1')
    model._layers[0] = input_layer
    print(model._layers[0])
    model.summary()
    # input_layer = tf.keras.layers.Input(shape=[None, None])
    # # input_layer = tf.keras.engine.InputLayer(input_shape=(272, 480, 3), name="input_1")
    # model._layers[0] = input_layer
    # in_l = model.layers[0]
    # print(in_l.input_shape)
    sys.exit()
    # input_layer = tf.keras.layers.Input(input_shape=(None, IMAGE_HEIGHT, IMAGE_WIDTH, 3), name="input_1")

    # print('============================')
    # concrete_func = model.signatures[
    #     tf.saved_model.DEFAULT_SERVING_SIGNATURE_DEF_KEY]
    # print('============================')
    # concrete_func.inputs[0].set_shape([None, IMAGE_HEIGHT, IMAGE_WIDTH, 3])
    # print('============================')
    ...
    tflite_model_path = path_cls.make_model_path(MAINNAME + '-' + DATASET + '_float16.tflite')
    ...
    # ========== save model ==========
    gen_cls = LTImageGenerator(LT_Generator_model=tflite_model_path,
...
with open(path_cls.make_csv_path('../../Total_time_'+CSV_NAME+'_cpu'+'.csv'), 'w') as f:
    ...
    for i in range(len(csv_files)):
        ...
        writer.writerow([csv_files[i][s:s+5], ave_str])
