...
# ************ クラス外呼び出し関数 ************
    ...
    def generator_loss(self, gen_output, target):
        ...
        total_gen_loss = (self.lambda_val * l1_loss)
...
# ========== NET ===========
    def _Generator(self):
        down_stack = [
            self._downsample(16, 4, apply_batchnorm=False),  # (bs, 128, 128, 64)
            self._downsample(32, 4),  # (bs, 64, 64, 128)
            self._downsample(64, 4),  # (bs, 32, 32, 256)
            self._downsample(128, 4),  # (bs, 16, 16, 512)
            self._downsample(128, 4),  # (bs, 8, 8, 512)
            self._downsample(128, 4),  # (bs, 4, 4, 512)
            self._downsample(128, 4),  # (bs, 2, 2, 512)
            self._downsample(128, 4),  # (bs, 1, 1, 512)
        ]
        up_stack = [
            self._upsample(128, 4, apply_dropout=True),  # (bs, 2, 2, 1024)
            self._upsample(128, 4, apply_dropout=True),  # (bs, 4, 4, 1024)
            self._upsample(128, 4, apply_dropout=True),  # (bs, 8, 8, 1024)
            self._upsample(128, 4),  # (bs, 16, 16, 1024)
            self._upsample(64, 4),  # (bs, 32, 32, 512)
            self._upsample(32, 4),  # (bs, 64, 64, 256)
            self._upsample(16, 4),  # (bs, 128, 128, 128)
        ]
        ...
        last = tf.keras.layers.Conv2DTranspose(self.output_channel, 4,
                                               ...
                                               activation='tanh')  # (bs, 256, 256, 3) tanh->=-1~1
