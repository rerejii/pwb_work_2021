...
class DataManager():
    ...
    def _random_crop(self, _sample_image, _answer_image, _weight_image, _dis_image, parse):
        ...
        cropped_image = tf.image.random_crop(stacked_image, size=[6, 512, 512])
        _sample_image = tf.stack([cropped_image[0], cropped_image[1], cropped_image[2]], axis=2)
        _answer_image = tf.expand_dims(cropped_image[3], axis=-1)
        _weight_image = tf.expand_dims(cropped_image[4], axis=-1)
        _dis_image = tf.expand_dims(cropped_image[5], axis=-1)
