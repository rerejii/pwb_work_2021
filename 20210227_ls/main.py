...
FIN_ACTIVATE = 'None'
#  '20200223'
OUT_ROOT_FOLDER_NAME = '20210222'
#  '20200223'
EPOCH_OUT_ROOT_FOLDER_NAME = '20210222'
#  DR*
...
# elif TASK == 'eva' or TASK =='eva_train-ds' or T:
    ...
    ds_iter = valid_ds_cls.get_inited_iter()
    img_idx = 0
    while valid_ds_cls.get_remain_data() != 0:
        input_image, target, weight, distance, w_path = next(ds_iter)
        gen_output = generator(input_image, training=False)
        print(gen_output)
        print(gen_output.shape)
        batch = gen_output.shape[0]
        gen_output = tf.cast(net_cls.binary_from_data(gen_output, label='output'), tf.uint8) * 255
        print(gen_output)
        for i in range(batch):
            byte = tf.image.encode_png(gen_output[i])
            out_path = 'Z:/hayakawa/binary/20210128/unet_ls_3/unet_ls_3-A/img/' + str(img_idx) + '_.png'
            tf.io.write_file(filename=out_path, contents=byte)
            img_idx += 1
