...
for dir_i in range(len(directory_set)):
    ...
    if not csv_files:
        print(directory + '/*/CsvDatas/step_accuracy.csv')
        print('no file')
        sys.exit()
    ...
    with open(out_csvpath, 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['ds', 'max_test_accuracy_index', 'train_accuracy', 'test_accuracy', 'valid_accuracy'])
        for i in range(len(csv_files)):
            ...
            ado_train_acc = df['train_accuracy'][max_test_acc_i]
            ...
            # グラフ線用
            train_ave_list.append(df['train_accuracy'].values[:epoch])
            test_ave_list.append(df['test_accuracy'].values[:epoch])
            valid_ave_list.append(df['validation_accuracy'].values[:epoch])
