...
class PathManager():
    ...
    def get_outroot_path(self):
        return self.output_rootfolder
    ...
    def search_checkpoint_path(self):
        # self.checkpoint_folder
        # print(self.checkpoint_folder + '/' + self.ckpt_step_name)
        tmp = glob.glob(self.checkpoint_folder + '/' + self.ckpt_step_name + '*.index')
        tmp = natsorted(tmp)
        out = [os.path.splitext(t)[0] for t in tmp]
        return out
# [int(n) for n in range(len(DEVICE_STR_LIST))]
