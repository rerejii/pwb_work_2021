...
class DataManager():
    ...
    def get_inited_iter(self):
        ...
        # if (self.suffle_buffer != -1):
        #     _dataset = _dataset.map(self._random_crop)
        #     _dataset = _dataset.map(self._random_rot4)
        #     _dataset = _dataset.map(self._random_filp)
        # else:
        #     _dataset = _dataset.map(self._cut_padding)
    ...
    def _random_crop(self, _sample_image, _answer_image, _weight_image, _dis_image, parse):
        stacked_image = tf.stack([_sample_image[:, :, 0], _sample_image[:, :, 1], _sample_image[:, :, 2],
                                  _sample_image[:, :, 3], _sample_image[:, :, 4],
                                  _answer_image[:, :, 0], _weight_image[:, :, 0], _dis_image[:, :, 0]], axis=0)
        cropped_image = tf.image.random_crop(stacked_image, size=[8, 256, 256])
        _sample_image = tf.stack([cropped_image[0], cropped_image[1], cropped_image[2], cropped_image[3], cropped_image[4]], axis=2)
        _answer_image = tf.expand_dims(cropped_image[5], axis=-1)
        _weight_image = tf.expand_dims(cropped_image[6], axis=-1)
        _dis_image = tf.expand_dims(cropped_image[7], axis=-1)
    ...
    def _random_rot4(self, _sample_image, _answer_image, _weight_image, _dis_image, parse): # 0.75 0.5 0.25
        random_n = tf.random.uniform([], seed=self.seed)
        if random_n > 0.75:
            _sample_image = tf.image.rot90(_sample_image)
            _answer_image = tf.image.rot90(_answer_image)
    ...
    def _random_filp(self, _sample_image, _answer_image, _weight_image, _dis_image, parse): # 0.75 0.5 0.25
        random_n = tf.random.uniform([], seed=self.seed)
        if random_n > 0.75:
            _sample_image = tf.image.flip_left_right(_sample_image)
            _answer_image = tf.image.flip_left_right(_answer_image)
            _weight_image = tf.image.flip_left_right(_weight_image)
            _dis_image = tf.image.flip_left_right(_dis_image)
            _sample_image = tf.image.flip_up_down(_sample_image)
            _answer_image = tf.image.flip_up_down(_answer_image)
            _weight_image = tf.image.flip_up_down(_weight_image)
            _dis_image = tf.image.flip_up_down(_dis_image)
        elif random_n > 0.50:
            _sample_image = tf.image.flip_left_right(_sample_image)
            _answer_image = tf.image.flip_left_right(_answer_image)
            _weight_image = tf.image.flip_left_right(_weight_image)
    ...
    # def _ds_cut_padding(self, target, padding, batch_shape=True):
    ...
    def _cut_padding(self, _sample_image, _answer_image, _weight_image, _dis_image, parse):
        _sample_image = _sample_image[self.ds_padding:-self.ds_padding, self.ds_padding:-self.ds_padding, :]
        _answer_image = _answer_image[self.ds_padding:-self.ds_padding, self.ds_padding:-self.ds_padding]
        _weight_image = _weight_image[self.ds_padding:-self.ds_padding, self.ds_padding:-self.ds_padding]
        _dis_image = _dis_image[self.ds_padding:-self.ds_padding, self.ds_padding:-self.ds_padding]
        return _sample_image, _answer_image, _weight_image, _dis_image, parse
    ...
    def _parse_function(self, _data):
        _features = {'sample_path': tf.io.FixedLenFeature((), tf.string, default_value=''),
                     ...
                     'hls_path': tf.io.FixedLenFeature((), tf.string, default_value=''),
    ...
    def _image_from_path(self, _parsed_features):
        ...
        _hls_png_bytes = tf.io.read_file(self.img_root + '/' + _parsed_features['hls_path'])
        _hls_image = tf.image.decode_png(_hls_png_bytes, channels=3, dtype=tf.dtypes.uint8, )
        ...
        return _sample_image, _answer_image, _weight_image, _dis_image, _hls_image, _parsed_features

    def _feature_scaling(self, _sample_image, _answer_image, _weight_image, _dis_image, _hls_image, _parsed_features):
        ...
        _sample_image, _answer_image, _weight_image, _hls_image = \
            self._netdata_normalize(_sample_image, _answer_image, _weight_image, _hls_image)
        # _hsv_image = tf.image.rgb_to_hsv(_sample_image)
        _sample_image = tf.concat([_sample_image, _hls_image[:, :, 1:]], -1)
        _dis_image = tf.cast(_dis_image, tf.float32)
        # if self.standardization:
        #     _std_list = _parsed_features['std_val']
    ...
    def _netdata_normalize(self, _sample_image, _answer_image, _weight_image, _hls_image):
        _sample_image = tf.cast(_sample_image, tf.float32)
        _answer_image = tf.cast(_answer_image, tf.float32)
        _weight_image = tf.cast(_weight_image, tf.float32)
        _hls_image = tf.cast(_hls_image, tf.float32)
        _sample_image = self.net_cls.netdata_from_img(_sample_image)
        _answer_image = self.net_cls.netdata_from_img(_answer_image)
        _weight_image = self.net_cls.netdata_from_img(_weight_image)
        _hls_image = self.net_cls.netdata_from_img(_hls_image)
        # _deepotsu_image = self.net_cls.netdata_from_img(_deepotsu_image, label='deepotsu_image')
        return _sample_image, _answer_image, _weight_image, _hls_image
